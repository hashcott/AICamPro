...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="procam",
        description="ProCam — xoá nền, filter và webcam ảo, tăng tốc bằng GPU AMD.")
...
def _check() -> int:
    ...
    if not info.is_gpu:
        print("  ⚠ Không thấy GPU qua ROCm — ProCam sẽ chạy trên CPU (rất chậm).")
...
def main(argv: list[str] | None = None) -> int:
    ...
    app.setApplicationName("ProCam")
    app.setApplicationDisplayName("ProCam")
