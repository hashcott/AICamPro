#!/usr/bin/env bash
# Tạo thiết bị webcam ảo cho AICamPro.
#
...
#      nên file của AICamPro phải sắp xếp sau cùng và phải gộp cả thiết bị của
#      những ứng dụng kia, nếu không sẽ làm hỏng webcam ảo của chúng.
set -euo pipefail

DEVICE_NR="${DEVICE_NR:-42}"
LABEL="${LABEL:-AICamPro Virtual Camera}"
# exclusive_caps=0 cho thiết bị của AICamPro.
...
#   Với =0 thiết bị luôn khai báo cả hai — AICamPro ghi được, OBS/Zoom/Meet thấy được.
#   Đặt EXCLUSIVE_CAPS=1 nếu ứng dụng nào đó của bạn đòi hỏi kiểu cũ.
EXCLUSIVE="${EXCLUSIVE_CAPS:-0}"
CONF="/etc/modprobe.d/zz-aicampro-v4l2loopback.conf"   # tên bắt đầu bằng zz để ghi đè các file khác
...
# dọn file của các bản cũ (thời app tên ProCam rồi Soi) để khỏi cấu hình chồng nhau
rm -f /etc/modprobe.d/procam-v4l2loopback.conf \
      /etc/modprobe.d/zz-procam-v4l2loopback.conf \
      /etc/modprobe.d/zz-soi-v4l2loopback.conf
cat > "$CONF" <<CONFEOF
# Do AICamPro tạo. Gộp toàn bộ thiết bị v4l2loopback của máy vào một chỗ vì
# modprobe chỉ dùng giá trị của file được đọc SAU CÙNG.
options v4l2loopback $PARAMS
CONFEOF
echo "v4l2loopback" > /etc/modules-load.d/zz-aicampro-v4l2loopback.conf
rm -f /etc/modules-load.d/procam-v4l2loopback.conf \
      /etc/modules-load.d/zz-procam-v4l2loopback.conf \
      /etc/modules-load.d/zz-soi-v4l2loopback.conf
