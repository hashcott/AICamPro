...
Mọi thứ chạy trong môi trường conda `procam` (Python 3.12 + PyTorch ROCm):

```bash
source ~/miniconda3/etc/profile.d/conda.sh && conda activate procam
```

| Việc | Lệnh |
|---|---|
| Chạy app | `./run.sh` (tự activate conda rồi `python -m procam`) |
...
| Cổng kiểm tra cú pháp nhanh | `python -m compileall -q procam` |
| Lint | `ruff check procam` (line-length 100, cấu hình trong `pyproject.toml`) |
...
- Kiểm tra GUI: chạy `python -m procam` rồi
  `import -window $(xdotool search --name 'ProCam — ' | head -1) out.png`.
  Cửa sổ có tên bắt đầu bằng `ProCam` nhưng chỉ cửa sổ `ProCam — …` là cửa sổ thật;
  còn một cửa sổ phụ 1×1 cùng tên `ProCam` (`import -window` sẽ thất bại trên nó).
- Chỉ có **một** camera vật lý, và nó không mở được hai lần. Tắt GUI trước khi
  ...
  (Đừng dùng `pkill -f "python -m procam"` — chuỗi đó khớp với chính lệnh shell
...
### Quy ước tensor trong `procam/gpu/`
...
`AppConfig` là cây dataclass, lưu ở `~/.config/procam/config.json`, preset ở
`~/.config/procam/presets/`. `_merge` khoan dung: bỏ qua khoá lạ, ép kiểu, không
...
- **QSS không hiểu mẹo vẽ tam giác bằng `border-left/right/top` của CSS.**
  ...
  một PNG vào `~/.cache/procam/` nên không phải kèm file ảnh vào repo — vì thế
...
- **modprobe đọc mọi file trong `/etc/modprobe.d` theo thứ tự abc, file SAU ghi đè
  ...
  của ProCam đặt tên `zz-…` để đọc sau cùng và phải **gộp** cả thiết bị của các
