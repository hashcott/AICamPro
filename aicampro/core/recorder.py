...
class Recorder:
    ...
    def start(self, out_dir: str, width: int, height: int, fps: int,
        ...
        path = _unique(directory / f"aicampro_{datetime.now():%Y%m%d_%H%M%S}.mp4")
...
def snapshot(bgr: np.ndarray, out_dir: str, alpha: np.ndarray | None = None) -> Path:
    ...
    if alpha is not None:
        path = _unique(directory / f"aicampro_{stamp}.png")
        cv2.imwrite(str(path), np.dstack([bgr, alpha]))
    else:
        path = _unique(directory / f"aicampro_{stamp}.jpg")
